...
io.sockets.on('connection', function(socket) { //open io connection
  sport.open(function(error) { //open serial connection
    if (error) {
      ...
      socket.on('bumpLeft', function (data) { //listen for toScreen fron socket 
        console.log("you bumped the " + data.l + " object!"); // log data
      });
      socket.on('bumpRight', function (data) { //listen for toScreen fron socket 
        console.log("you bumped the " + data.r + " object!"); // log data
      ...
      sport.on("data", function (data) {
        ...
        socket.emit('serialText', {
          s: data
        }); //emit to screen the data recived from socket io
